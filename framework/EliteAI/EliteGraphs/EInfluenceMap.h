...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		if (m_TimeSinceLastPropagation >= m_PropagationInterval)
		{
			...
			for (auto node : nodes)
			{
				auto nodeConnections = GetNodeConnections(node);

				float biggestInfluence{};
				for (auto connection : nodeConnections)
				{
					float newInfluence = GetNode(connection->GetTo())->GetInfluence() * expf(-connection->GetCost() * m_Decay);

					if (abs(newInfluence) > abs(biggestInfluence))
					{
						biggestInfluence = newInfluence;
					}
				}

				float selfInfluence = node->GetInfluence() * expf(-m_Decay);
				if (abs(selfInfluence) > abs(biggestInfluence))
				{
					biggestInfluence = selfInfluence;
				}

				float Influence = Elite::Lerp(node->GetInfluence(), biggestInfluence, m_Momentum);

				if (Influence > m_MaxAbsInfluence)
				{
					Influence = m_MaxAbsInfluence;
				}

				m_InfluenceDoubleBuffer.push_back(Influence);
			}
